  ...
  module Define
    def eval_define(exp, env)
      ...
      if var_ref != nil
        ...
        extend_env!([var], [_eval(var, env)], env)
      end
  ...
  module Repl
    ...
    def pp(exp)
      if exp.is_a?(Symbol) or num?(exp)
        ...
        "(closure #{pp(parameter)} #{pp(body)} #{pp(env)})"
      elsif exp.is_a?(Array) and lambda?(exp)
        parameter, body = exp[1], exp[2]
        "(lambda #{pp(parameter)} #{pp(body)})"
      elsif exp.is_a?(Hash)
...
class Scheme
  ...
  include SchemeR::Quote
  include SchemeR::Parser
  include SchemeR::Repl
end
